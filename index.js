...
function generateGuessList(beforeOrAfter, guesses, word) {
    const guessList = [];
    guesses.filter(getBeforeOrAfterGuesses).forEach((guess) => {
...
function setGuess(event) {
    ...
    if (this.guessError) {
        // only re-evaluate form validity on-the-fly if it was invalid
...
function shouldShowSubmitName() {
    return (
        this.winTime && this.guesses.length > 0 && !this.isReplay && !this.submitTime && isPlayDateToday(this.playDate)
    );
}
...
function handleBadResponse(json, responseStatus) {
    const invalidReason = json?.invalidReason || `${UNKNOWN_LEADERBOARD_ERROR} ${responseStatus}`;
...
function unshiftUniqueValue(arrayOfUniqueValues, newValue) {
    return [newValue].concat(arrayOfUniqueValues.filter((value) => value !== newValue));
}
...
const SHORT_WEEK_DAY_BY_INDEX = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
const SHORT_MONTH_BY_INDEX = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
